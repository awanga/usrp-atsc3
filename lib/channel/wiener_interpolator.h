...
public:
    explicit WienerInterpolator(
        const WienerInterpolatorConfig& config = WienerInterpolatorConfig());
    ~WienerInterpolator();
    ...
    std::vector<sample_t> interpolate(const std::vector<sample_t>& pilot_estimates,
    ...
    std::vector<sample_t> interpolate_with_history(const std::vector<sample_t>& pilot_estimates,
    ...
    const WienerInterpolatorConfig& get_config() const {
        return config_;
    }

    // Get filter coefficients (for debugging/analysis)
    const std::vector<float>& get_freq_coefficients() const {
        return freq_coeffs_;
    }
    const std::vector<float>& get_time_coefficients() const {
        return time_coeffs_;
    }
...
private:
    ...
    void interpolate_frequency(const std::vector<sample_t>& pilot_estimates,
                               const std::vector<size_t>& pilot_positions,
    ...
    sample_t apply_fir(const std::vector<sample_t>& input, size_t center_idx,
...
std::vector<float> compute_wiener_coefficients(float doppler_hz, float delay_spread_us,
                                               float snr_db, size_t num_taps);
