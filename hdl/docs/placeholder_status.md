...
|---|---|---|---|---|
| 1 | LDPC H-matrices | `lib/fec/ldpc_decoder.cc` | Non-spec placeholder matrices (documented in the LDPC decoder RTL notes) | Standardized per-rate QC-LDPC matrices, A/322 Annex |
...
- **Bootstrap detector correlation** (`lib/sync/bootstrap_detector.cc`):
  ...
  since that's what bit-exactness is measured against, not because it's
  spec-accurate.
...
- **`check_reassembly_timeouts()`** (`lib/framing/alp_demux.cc:440-452`)
  ...
  as the not-yet-built config sequencer).
